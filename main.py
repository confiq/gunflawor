...
class Benchmark(object):
        ...
        for retries in range(1, 10):
            try:
                ...
                if not res and not res.ok:
                    logger.error('The container did not return OK code. This is probably due misconfiguration of the '
                                 'worker. Result: {}'.format(res.content))
                break
            except requests.exceptions.ConnectionError:
    ...
    def stress_test(self):
        res = boom.load('http://127.0.0.1:{}/'.format(WORKING_PORT), BOOM_REQUESTS, BOOM_CONCURRENCY, None, 'GET', '',
                        ct='text/plain', auth=None, quiet=True)
